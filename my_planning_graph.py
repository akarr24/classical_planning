...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        return any([~a in actionA.preconditions for a in actionB.effects]) or any([~a in actionB.preconditions for a in actionA.effects])
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        sum = 0
        for i in self.goal:
            for id, layer in enumerate(self.literal_layers):
                if i in layer:
                    sum += id
                    break
        return sum
    ...
    def h_setlevel(self):
        ...
        #raise NotImplementedError
        def goalDiscovered(layer):
            for i in self.goal:
                if i not in layer:
                    return False
            return True
        
        def mutexAbsent(layer):
        ...
        while not self._is_leveled:
            ...
            if goalDiscovered(layer) and mutexAbsent(layer):
                return level
